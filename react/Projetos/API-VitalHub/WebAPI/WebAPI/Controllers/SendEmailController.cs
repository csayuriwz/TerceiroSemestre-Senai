    ...
    public class SendEmailController : ControllerBase
    {
        ...
        public async Task<IActionResult> SendMail(string email, string userName)
        {
            try
            {
                ...
                mailRequest.Subject = "Ola esse eh um email vindo da turma de dev";

                //parte visual do email (corpo), chamar um metodo para renderizar um html
                mailRequest.Body = GetHtmlContent(userName);

                //chama o metodo que enviara o email
                await emailService.SendEmailAsync(mailRequest);

                return Ok("Email enviado com suceero");
            }
            catch (Exception)
            {
